...
## 🚨 MUST FOLLOW RULES (개발 필수 준수 규칙)

**UI/Frontend 변경 작업 시 필수 준수사항:**

1. **기존 API들의 입력/출력 포맷을 절대 수정하지 않는다.**
   - API 엔드포인트 경로, 요청/응답 형식은 기존과 동일하게 유지
   - 새로운 필드 추가는 하위 호환성을 유지하는 선에서만 허용

2. **기존 SQL 구문들을 절대 수정하지 않는다.**
   - 데이터베이스 스키마 및 쿼리는 기존 구조 유지
   - 새로운 테이블/컬럼 추가 시에도 기존 쿼리에 영향 없도록 설계

3. **기존 DOM 구조를 절대 수정하지 않는다.**
   - HTML 구조, ID, class 이름은 기존과 동일하게 유지
   - 기존 JavaScript가 참조하는 DOM 요소는 변경 금지

4. **기존 SCSS 스타일 내지는 SVG 파일을 수정하지 않는다.**
   - 기존 스타일 파일은 그대로 유지
   - 새로운 스타일 추가 시 별도 파일로 분리

5. **기존 API/SQL/DOM 바꿔야 할 시 반드시 바꾸는 이유 설명 후 사용자의 허가 후 진행.**
   - 변경 필요성, 영향 범위, 대안을 명확히 설명
   - 사용자 승인 없이는 절대 변경 금지

6. **수정 내역 저장 및 commit 시 반드시 해당 커밋에 대한 git patch 파일과 수정위치 및 이유를 설명한 문서를 생성해야한다.**
   - 모든 커밋은 patch 파일과 함께 문서화
   - 변경 사유, 위치, 영향 범위를 명시한 CHANGELOG 작성

---

**주요 변경사항 (v3.2)**:
- 🚨 MUST FOLLOW RULES 섹션 추가 (UI/Frontend 개발 필수 준수 규칙)
- 🎨 새로운 UI (new-exgpt-ui) 통합 준비

...
async def get_optional_current_user_from_session(
    session_id: str = Cookie(None, alias="JSESSIONID"),
    db: AsyncSession = Depends(get_db)
) -> Optional[dict]:
    """
    HTTP 세션에서 사용자 정보 조회 (Optional)

    세션이 없거나 만료되어도 예외를 던지지 않고 None을 반환합니다.
    파일 업로드처럼 세션이 선택적인 경우 사용합니다.
    ...
    """
    if not session_id:
        logger.info("No session ID - allowing anonymous access")
        return None
    ...
    try:
        ...
        if not user_data:
            logger.info(f"Session not found - allowing anonymous access: {session_id}")
            return None
        ...
        logger.warning(f"Session data parse error - allowing anonymous: {e}")
        return None
    except Exception as e:
        logger.warning(f"Session validation error - allowing anonymous: {e}")
        return None

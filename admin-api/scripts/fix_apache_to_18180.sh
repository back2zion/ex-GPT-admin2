#!/bin/bash
# Apache 설정을 Tomcat 18180 포트로 수정

set -e

echo "======================================"
echo "Apache 설정 수정: Port 18180 (Tomcat)"
echo "======================================"
...
# 2. 설정 파일 수정
echo "2. Apache 설정 파일 작성 중 (Port 18180)..."
...
<VirtualHost *:20443>
    ...
    # Tomcat WAS 프록시 (채팅, 파일, 공지사항, 만족도 등 - Port 18180)
    ProxyPass /exGenBotDS/ http://localhost:18180/exGenBotDS/
    ProxyPassReverse /exGenBotDS/ http://localhost:18180/exGenBotDS/
...
echo "======================================"
echo ""
echo "✅ Apache → Tomcat (Port 18180) 프록시 설정 완료"
echo ""
echo "포트 확인:"
netstat -tuln | grep 18180 || ss -tuln | grep 18180
echo ""

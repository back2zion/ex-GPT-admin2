...
class SessionQueueService:
    ...
    async def create_session(self, user_id: str, session_id: str) -> Dict:
        """
        새 세션 생성 (Race Condition 방지 - Lua 스크립트 사용)
        ...
        # Lua 스크립트: 원자적으로 세션 생성 (Race Condition 방지)
        lua_script = """
        ...
        result = await redis.eval(
            lua_script,
            1,  # KEYS 개수
            self.ACTIVE_SESSIONS_KEY,  # KEYS[1]
            user_id,  # ARGV[1]
            session_data_json,  # ARGV[2]
            str(self.MAX_ACTIVE_SESSIONS)  # ARGV[3]
        )
        ...
        if result == 1:
            ...
            logger.info(f"세션 생성: user_id={user_id}, session_id={session_id}, active={active_count}/{self.MAX_ACTIVE_SESSIONS}")

...
// 항상 상대 경로 사용 (nginx/Apache reverse proxy 환경)
const API_BASE = '/api/v1';

console.log('[dataProvider] API_BASE 초기화:', API_BASE);
console.log('[dataProvider] window.location:', window.location.href);
...
const httpClient = async (url, options = {}) => {
    ...
    // fetch 직접 사용 (credentials 포함으로 세션 쿠키 전송)
    ...
    if (!response.ok) {
        const errorDetail = json.detail || json.message || `HTTP ${response.status}: ${response.statusText}`;
        console.error('[dataProvider] HTTP Error:', {
            url,
...
const dataProvider = {
    ...
    getList: async (resource, params) => {
        ...
        // vector_documents 리소스 (특별 처리)
        if (resource === 'vector_documents') {
            const query = new URLSearchParams({
                skip: (page - 1) * perPage,
                limit: perPage,
                sort_by: field || 'id',
            ...
            return {
                data: json.items.map(item => ({ ...item, id: String(item.id) })),
                total: json.total,
            };
        }

        // error_reports 리소스 (특별 처리)
        if (resource === 'error_reports') {
            const query = new URLSearchParams({
                skip: (page - 1) * perPage,
                limit: perPage,
                sort_by: field || 'id',
            ...
            const url = `${API_BASE}/admin/error_reports?${query}`;
            const { json } = await httpClient(url);
        ...
        // recommended_questions 리소스 (특별 처리)
        if (resource === 'recommended_questions') {
            const query = new URLSearchParams({
                skip: (page - 1) * perPage,
                limit: perPage,
                sort_by: field || 'id',
            ...
            const url = `${API_BASE}/admin/recommended_questions?${query}`;
            const { json } = await httpClient(url);
        ...
        const url = `${API_BASE}/admin/${endpoint}?${query}`;
        const { json } = await httpClient(url);
    ...
    getOne: async (resource, params) => {
        ...
        if (resource === 'conversations') {
            ...
            const endpoint = resourceToEndpoint(resource);
            url = `${API_BASE}/admin/${endpoint}/${params.id}`;
        }
    ...
    getMany: async (resource, params) => {
        ...
        const promises = params.ids.map(id =>
            httpClient(`${API_BASE}/admin/${endpoint}/${id}`)
        );
